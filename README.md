...
ReadyMCAT is a fork of [Anki](https://apps.ankiweb.net) turned into a study app for the **MCAT** (the Medical College Admission Test). It is not a plugin or add-on — the changes live in Anki's own Rust engine, Python/Qt desktop app, Svelte/TypeScript front end, and a new iOS companion, so the desktop and phone share one engine. The full product rationale is in [`docs/ReadyMCAT-PRD.md`](./docs/ReadyMCAT-PRD.md) and the research behind it in [`docs/brainlift-mcat.md`](./docs/brainlift-mcat.md).
...
**Prerequisites (desktop).** Install these three once; the build downloads
everything else (Python via `uv`, Node/Yarn, and the protobuf compiler `protoc`)
automatically into `out/` — you do **not** install those by hand:

- **Rust toolchain** via [rustup](https://rustup.rs). The exact version pinned in
  [`rust-toolchain.toml`](./rust-toolchain.toml) is fetched automatically on the
  first build.
- **Ninja or N2** — run `./tools/install-n2` (N2 gives nicer status output), or
  install Ninja 1.10+ from your package manager / [releases](https://github.com/ninja-build/ninja/releases).
- **[`just`](https://just.systems)** command runner — `brew install just` (macOS)
  or `uv tool install just`.
- **Platform notes:** macOS needs the Xcode command-line tools
  (`xcode-select --install`) plus `git`/`rsync`, and `brew install mpv lame` for
  audio; see [docs/mac.md](./docs/mac.md), [docs/linux.md](./docs/linux.md),
...
The full MCAT question bank (1,075 cards across four decks) **pre-loads
automatically on first launch — there is no import step and no external deck to
download.** The introductory diagnostic, `taxonomy.json` and the teach-on-miss
`subquestions.json` ship in the repo and are provisioned next to the new
profile's collection for you, so a fresh clone runs out of the box. Use
`just run-optimized` for a release build, and `./tools/build-installer` for an
(unsigned) macOS `.dmg`. Full dev setup: [docs/development.md](./docs/development.md).
