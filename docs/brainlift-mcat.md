...
- SPOV 1 → **teach-on-miss** (the core feature): authored ladders on every bundled question + spaced re-retrieval, shipped on desktop with no runtime AI; runtime AI auto-generation is deferred. Instrumented by the corrected-concept re-retrieval rate; the full ablation is the fair test still ahead.
