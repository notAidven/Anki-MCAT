...
| **Reviewers** — open a format        | `SetCurrentDeck` (7/22) → `GetQueuedCards` (13/3); MCQ/Passages first isolate via a filtered deck (`GetOrCreateFilteredDeck` 7/19 → `AddOrUpdateFilteredDeck` 7/20 → `RebuildFilteredDeck` 13/16, emptied with `EmptyFilteredDeck` 13/15) | one format's cards only (never a nested sibling)                                                                        |
...
content types, then raises the daily new/review limits and selects
`ReviewCardOrder::PointsAtStake` (order 13). The deck layout is left **exactly**
as `build_question_bank.py` builds it — the canonical layout the desktop uses and
the phone receives over sync: MCQ in the top-level `ReadyMCAT` deck, plus
`ReadyMCAT::Free Response`, `ReadyMCAT::Passages` and `ReadyMCAT::Passages::CARS`.
Because MCQ shares the `ReadyMCAT` parent with the other formats, the app
isolates a one-tap MCQ (and Passages) study session with a small reused filtered
deck (`AppModel.studyDeckId`, mirroring the desktop hub) instead of relocating
cards — so each tile's count stays child-excluding (MCQ 414 · FR 410 · Passages
174 · CARS 77) and studying MCQ never pulls in a sibling format.
...
same tolerant MCQ parser (`parseLadder`) for rungs shaped
`{question, options, correctIndex, explanation}`, and the same three
deterministic guardrails with the same constants —

| Guardrail   | Rule                                                             | Constant                                          |
| ----------- | ---------------------------------------------------------------- | ------------------------------------------------- |
| schema      | 2–4 well-formed MCQ rungs (3–4 distinct options, in-range index) | `minRungs=2 maxRungs=4 minOptions=3 maxOptions=4` |
| answer-leak | rung 1 stem/correct option must not restate the answer (lexical) | `answerLeakMax=0.7`                               |
| grounding   | each rung's correct option + explanation traces to the card      | `groundingMin=0.5`                                |
...
(`GeneratedLadderView.swift`): each guiding rung is an interactive
multiple-choice question (reusing the same `ChoiceQuestion` component as the
authored reviewers) — select an option → correct/incorrect feedback +
explanation → next rung — and after the last rung the original card is re-asked
and graded (first-try → Good; after-ladder → Again + `ReadyMCAT::struggling`).
...
```
[1] deck_tree OK: {'ReadyMCAT': (414, 414), 'ReadyMCAT::Free Response': (410, 410), …}
[1b] MCQ isolation OK: filtered deck holds 414 MCQ-only cards
...
[7] empty_filtered_deck OK: launcher returned its cards home
...
- Native **AI teach-on-miss generation**: for a missed card that has **no
  ...
  (`LadderGen.swift`) builds a grounded multiple-choice ladder via a
  **server-side proxy** (a Cloudflare Worker — no OpenAI key on the phone) and
