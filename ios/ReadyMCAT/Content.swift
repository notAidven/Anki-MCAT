...
enum Format: String, CaseIterable, Identifiable {
    ...
    /// Fully-qualified deck name in the canonical ReadyMCAT collection — the
    /// exact layout readymcat/tools/build_question_bank.py builds and the phone
    /// receives over sync (MCQ_DECK_NAME / FR_DECK_NAME / PASSAGE_DECK_NAME /
    /// CARS_PASSAGE_DECK_NAME). MCQ lives DIRECTLY in the top-level `ReadyMCAT`
    /// deck, which is ALSO the parent of Free Response / Passages / CARS; the
    /// tile count reads that deck's child-excluding counters, and MCQ study is
    /// isolated to the MCQ notetype via a filtered deck (see AppModel
    /// .studyDeckId / isolatingSearch), so it never pulls a sibling format in.
    var deckName: String {
        switch self {
        case .mcq: return "ReadyMCAT"
        case .fr: return "ReadyMCAT::Free Response"
        case .passage: return "ReadyMCAT::Passages"
        case .cars: return "ReadyMCAT::Passages::CARS"
        }
    }
