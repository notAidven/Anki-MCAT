...
struct ReviewSessionView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // `studyDeckId` isolates formats with nested children (mcq, passage)
            // through a filtered deck so the session serves ONLY this format;
            // the AI-demo override, when set, wins.
            guard let deckId = deckIdOverride ?? model.studyDeckId(for: format) else {
    ...
    private func close() {
        // Return any cards the launcher filtered deck pulled in (mcq/passage) to
        // their home decks so the format tiles read honest counts, THEN refresh.
        model.returnLauncherCards()
