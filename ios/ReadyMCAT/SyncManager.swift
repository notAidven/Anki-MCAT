...
final class SyncManager: ObservableObject {
    ...
    /// The ReadyMCAT sync account. Both the desktop and this app log in with the
    ...
    /// `SYNC_USER1=<user>:<pass>`, so the "email" is simply the username string
    /// (no real Google/OAuth). Credentials intentionally start blank. A
    /// production build must store user-entered credentials in the Keychain.
    enum Default {
        static let endpoint = "http://127.0.0.1:27701/"
        static let username = ""
        static let password = ""
    }
    ...
    init() {
        ...
        endpoint = d.string(forKey: Key.endpoint) ?? Default.endpoint
        username = d.string(forKey: Key.username) ?? Default.username
        password = d.string(forKey: Key.password) ?? Default.password
    }
