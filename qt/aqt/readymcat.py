...
class ReadyMCATDashboardView:
    ...
    def _on_bridge_cmd(self, cmd: str) -> bool:
        ...
        if cmd == "openHome":
            ...
            # The shared "Study next" card can launch a format straight from the
            # dashboard, the same one-tap promise as the home hub's tiles.
            # ``start_deck_review`` is a no-op if the deck/collection is missing.
            from aqt.readymcat_home import start_deck_review

            start_deck_review(self.mw, cmd.split(":", 1)[1])
...
def _bundled_taxonomy_path(mw: aqt.main.AnkiQt) -> str:
    ...
    return ""); otherwise hand the dashboard/home hub an explicit path to the
    bundled file so it resolves in a fresh clone / `just run` build even before
    provisioning has copied it next to the collection. Mirrors
    ``_bundled_quiz_path``: repo layout first (``qt/aqt/readymcat.py`` -> repo
    root -> ``taxonomy.json``), then the current working directory. Returns ""
    when none is found (the backend may still find one next to the collection)."""
    if mw.col is not None:
        col_dir = os.path.dirname(mw.col.path)
        if os.path.exists(os.path.join(col_dir, "taxonomy.json")):
            return ""
    # repo layout: qt/aqt/readymcat.py -> repo root -> taxonomy.json
    repo_root = Path(__file__).resolve().parents[2]
    for candidate in (repo_root / "taxonomy.json", Path.cwd() / "taxonomy.json"):
        try:
...
@dataclass
class Concept:
    """One concept and its guiding ladder.

    Authored concepts (``subquestions.json``) carry ``{"q", "a"}`` reveal-and-
    self-mark rungs; a runtime-generated concept carries interactive MCQ rungs
    (``{"question", "options", "correctIndex", "explanation"}``). ``ladder`` is
    typed loosely so it can hold either shape; the reviewer renders per rung.
    """
