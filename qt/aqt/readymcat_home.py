...
def _return_launcher_cards(mw: aqt.main.AnkiQt) -> None:
    """Return any cards the reused launcher filtered deck is still holding to
    their home decks.

    A one-tap tile launch (``start_deck_review`` for ``mcq``/``passage``) pulls
    a format's cards INTO the ``ReadyMCAT Launcher`` filtered deck; while they
    sit there, that format's home deck (e.g. the MCQ deck) reports a depleted —
    often **zero** — due/total count, so the hub tile would wrongly read 0 after
    you've studied from it once. Emptying the filtered deck returns the cards and
    keeps the tile counts honest. Main-thread only (it mutates the collection)."""
    if mw.col is None:
        return
    try:
        ...
        print("ReadyMCAT: could not return launcher cards", exc)
...
class ReadyMCATHomeView:
    ...
    def show(self) -> None:
        ...
        First returns any cards the one-tap launcher filtered deck is still
        holding to their home decks, so the per-format tile counts are honest:
        studying a format from its tile pulls that format's cards into the
        filtered deck, which would otherwise make the tile read 0 (see
        ``_return_launcher_cards``)."""
        _return_launcher_cards(self.mw)
        self.web.set_bridge_command(self._on_bridge_cmd, self)
        self._loads += 1
