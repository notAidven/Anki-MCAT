...
**multiple-choice** sub-questions at runtime (each rung is
`{question, options, correctIndex, explanation}`, so the student works it out by
choosing) so the retrieval-first correction reaches every card, not just the
pre-loaded bank. Everything else in the app (the bank, the diagnostic, the three
scores) runs with **no model calls**.
...
| -------------------------------------------------------- | ----------------- | --------------------------- |
| Accuracy (passes guardrails + judge cutoff)              | **1.0** (100%)    | 0.188 (18.8%)               |
| Wrong-answer rate (leak / wrong / off-topic / no ladder) | **0.0** (0%)      | 0.625 (62.5%)               |
| Source-grounding mean                                    | **0.809**         | 0.33                        |
| Grounding pass-rate                                      | **1.0** (100%)    | 0.25 (25%)                  |
| Judge mean (1–5)                                         | **4.875**         | 2.688                       |
| Schema pass-rate                                         | 1.0 (100%)        | 1.0 (100%)                  |
| Answer-leak rate                                         | 0.0               | 0.0                         |
| Win-rate vs baseline (head-to-head)                      | **0.75** (75%)    | —                           |

_(n = 16 held-out cards; judge calibration `agreement = 1.0` over 2 hand-labelled
cases; **live MCQ** run 2026-07-05, `genModel: gpt-4o-mini`, `judgeModel: gpt-4o`.
It is a live LLM run, so it varies: across three back-to-back runs grounding
pass-rate stayed 100% and win-rate 75% with all 7 gates PASS, while accuracy
ranged 81.2–100% and wrong-answer 0–18.8%.)_
...
The authored bank is `{q,a}`, so the retrieved ladder is reshaped into the **same
MCQ form** the generator emits (`authored_to_mcq`: authored sub-answer → correct
option + explanation, sibling answers → distractors) and then scored on the
missed card by the **same** guardrails + judge — so the comparison is
apples-to-apples under the MCQ schema, and what shows through is the baseline's
genuine off-topic weakness, not a format mismatch. Because the authored bank is
disjoint from the held-out set's identifiers and the generator prompt is
**zero-shot**, there is no train/test leakage.
