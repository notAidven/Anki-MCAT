...
[`../docs/ReadyMCAT-PRD.md`](../docs/ReadyMCAT-PRD.md).

> **Clone-and-run:** everything here ships in the repo. After `just run` (see
> the [top-level README](../README.md#build--run-both-apps) for the one-time
> prerequisites), a brand-new profile is provisioned automatically — the decks
> are built and `taxonomy.json`, `subquestions.json` and `diagnostic_quiz.json`
> are copied next to the new collection. **No external deck and no manual
> file-copying are required.**
...
- `ts/reviewer/teach_on_miss.ts` — the in-card UI (exposed globally as `_teachOnMissStart`
  via `index.ts`/`index_wrapper.ts`); drives the ladder → main re-show → outcome and reports
  over `bridgeCommand`. It renders each rung by shape: an **authored** `{q,a}` rung as
  attempt → reveal → self-mark, and an **AI-generated MCQ** rung
  (`{question, options, correctIndex, explanation}`) as an interactive multiple-choice step
  (select an option → correct/incorrect feedback + explanation → next), reusing the same MCQ
  interaction pattern as `ts/reviewer/mcq.ts`. Either way the card's real answer stays hidden
  until the ladder is done (retrieve-before-reveal).
