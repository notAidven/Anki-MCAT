...
teach-on-miss ladders** (the guiding **multiple-choice** sub-questions —
`{question, options, correctIndex, explanation}` — shown when a student misses a
card that has no authored ladder). Because it is AI, its quality is measured here
rather than assumed — and measured _before_ a student ever sees a generated
ladder.
...
1. **Deterministic guardrails** — the exact validators from
   ...
   what actually ships:
   - **schema**: 2–4 well-formed multiple-choice rungs
     (`{question, options (3–4), correctIndex, explanation}`);
   - **answer-leak**: the first rung must not restate the card's answer;
   - **source-grounding**: each rung's **correct option + explanation** must
     trace back to the card's own material (question + answer + cited source).
...
3. **Beats-a-baseline** — the generated ladder is compared against the PRD's
   ...
   nearest _authored_ sub-question ladder from `subquestions.json`. The authored
   bank is `{q,a}`, so each retrieved ladder is reshaped into the **same MCQ
   form** the generator emits (`authored_to_mcq`: the authored sub-answer becomes
   the correct option + explanation, its sibling answers become the distractors)
   and scored on the missed card by the **same** guardrails + judge — so the
   comparison is apples-to-apples under the MCQ schema. What shows through is the
   baseline's **genuine** weakness (its retrieved ladder is grounded in _another_
   concept's material, not the missed card's), not a format mismatch.
...
- `report.json` — the captured machine-readable report from the latest run
  (its `mode` field stamps whether the numbers are `live` or `stub`). The
  committed copy is a **live MCQ** run (2026-07-05): AI accuracy 100%,
  wrong-answer 0%, grounding pass-rate 100% (mean 0.809), judge 4.88, win-rate
  75% vs the retrieval baseline — all 7 gates PASS. Being a live LLM run it
  varies slightly between runs; across three back-to-back runs the grounding gate
  and win-rate held (100% grounding pass, 75% win, all gates PASS each time)
  while accuracy ranged 81–100% and wrong-answer 0–19% (n = 16).
