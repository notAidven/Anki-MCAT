...
3. Beats-a-baseline - the generated ladder is compared against the PRD's
   ...
   nearest *authored* ladder from ``subquestions.json``. The retrieved ladder
   is scored through the SAME layers (deterministic guardrails + judge) as the
   AI, so the comparison is apples-to-apples.
...
# reuse it verbatim. It has no notion of the missed card's own material, so its
# ladder is grounded in ANOTHER concept's source, not this card's - which is
# why it collapses on held-out cards with no close authored analog. It is
# scored by the SAME guardrails + judge as the AI, so the side-by-side is fair.
...
def stub_generator(messages: list[dict[str, str]], model: str) -> str:
    ...
    ladder from answer keywords so the pipeline runs without a network call.
    Not meant to be high quality - just deterministic and well-formed."""
    ...
    ladder = [
        {"q": "What background terms does this question involve?", "a": first},
        {"q": "How do those combine to reach the answer?", "a": second},
    ]
...
def stub_judge(messages: list[dict[str, str]], model: str) -> str:
    ...
    if problems:
        ...
        first = usable[0]
        first_text = f"{first.get('q', '')} {first.get('a', '')}"
...
def eval_card(
    ...
    # Baseline ladder: nearest authored concept's ladder, reused verbatim and
    # scored on THIS card by the identical guardrails + judge.
    concept, sim = retriever.retrieve(
        f"{card.get('question', '')} {card.get('answer', '')}"
    )
    base_ladder = list(concept.get("ladder", [])) if concept else None
