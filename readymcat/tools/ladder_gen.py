...
The generated ladder uses the same ``{"q", "a"}`` rung shape as the
``subquestions.json`` sidecar, so the existing teach-on-miss reviewer renders
it with no changes.
...
#: words — otherwise the ladder just reveals the answer instead of scaffolding
#: toward it. Leak when containment of the answer in rung 1 meets this.
ANSWER_LEAK_MAX = 0.7
...
def parse_ladder(raw_completion: str) -> Optional[list[dict[str, str]]]:
    """Extract a ``[{q, a}]`` ladder from a model completion.

    Tolerant of code fences / stray prose around the JSON. Returns a
    normalized list of ``{"q", "a"}`` string rungs, or ``None`` when nothing
    parseable is found. Malformed rungs are dropped rather than crashing —
    the reviewer then simply runs a shorter (or, if empty, no) ladder.
    """
...
def check_schema(ladder: object) -> list[str]:
    """Structural problems with a parsed ladder (empty list == valid)."""
    problems: list[str] = []
    ...
    for i, rung in enumerate(ladder):
        if not isinstance(rung, dict):
            ...
            continue
        if not str(rung.get("q", "")).strip():
            problems.append(f"rung {i} has an empty question")
        if not str(rung.get("a", "")).strip():
            problems.append(f"rung {i} has an empty answer")
    return problems


def check_answer_leak(ladder: list[dict[str, str]], context: CardContext) -> bool:
    """True when the FIRST rung *blatantly* gives away the final answer.

    Two triggers: the answer text appears near-verbatim inside rung 1, or a
    high fraction (``ANSWER_LEAK_MAX``) of the answer's content words are
    already present in rung 1. This is deliberately a coarse, high-precision
    gate: a bag-of-words comparison cannot reliably tell a conclusion-first
    rung from a prerequisite-first rung that happens to share vocabulary, so
    subtle "hands it over" leaks are left to the eval harness's LLM-as-judge
    (the reason the eval is hybrid). The generation prompt also instructs the
    model not to reveal the answer on rung 1, so subtle leaks are rare.
    """
    if not ladder:
        return False
    first = ladder[0]
    first_text = f"{first.get('q', '')} {first.get('a', '')}".strip()
...
def grounding_score(ladder: list[dict[str, str]], context: CardContext) -> float:
    """Mean fraction of each sub-answer's content words supported by the
    card's own material. 1.0 == fully grounded, 0.0 == invented."""
    if not ladder:
        return 0.0
    grounding = context.grounding_text
    scores = [_containment(rung.get("a", ""), grounding) for rung in ladder]
    return sum(scores) / len(scores) if scores else 0.0


def validate_ladder(
    ladder: Optional[list[dict[str, str]]], context: CardContext
) -> ValidationResult:
