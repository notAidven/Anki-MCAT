...
To go the OTHER way — strip all synthetic demo data back out of a profile so the
dashboard reflects only REAL study — pass ``--clear-demo`` (close the desktop app
first so the collection is not open elsewhere)::

    out/pyenv/bin/python readymcat/tools/seed_demo_dashboard.py \
        --collection /path/to/collection.anki2 --clear-demo

This removes the demo notes, their cards, AND the synthetic reviews (see
``remove_demo_data`` for why the reviews must go too).
...
def remove_demo_data(col: "Collection") -> int:
    """Delete every synthetic demo note, its cards, AND the synthetic reviews
    those cards accumulated. Returns the number of notes removed.

    Anki deliberately KEEPS a card's ``revlog`` history when the card itself is
    deleted (review history survives for statistics). For synthetic demo data
    that is exactly wrong: the hundreds of invented reviews would linger as
    orphaned rows that still count toward the dashboard's give-up threshold
    (``graded_reviews`` in ``rslib/src/points_at_stake``), so the honest-scores
    dashboard would keep thinking it "has enough data" and report **Memory as 0%
    over zero real graded cards**. We therefore also delete the revlog rows for
    the demo cards, leaving a clean real-data-only slate.

    This is precise and safe: the deleted revlog rows belong ONLY to synthetic
    demo cards (found by ``tag:DEMO_TAG``), so a real student's real reviews —
    which live on their real cards — are never touched.
    """
    note_ids = list(col.find_notes(f"tag:{DEMO_TAG}"))
    if not note_ids:
        # Nothing seeded; still clear a stale marker from a partial prior run.
        try:
            col.remove_config(DEMO_CONFIG_KEY)
        except Exception:
            pass
        return 0
    # Collect the demo cards BEFORE removing their notes (afterwards the cards
    # are gone and can no longer be found), so their synthetic reviews can be
    # purged by card id.
    card_ids = [int(cid) for cid in col.find_cards(f"tag:{DEMO_TAG}")]
    col.remove_notes(note_ids)
    # Delete the synthetic reviews in chunks (keeps well under SQLite's bound-
    # parameter limit even for a large reseed).
    for start in range(0, len(card_ids), 400):
        chunk = card_ids[start : start + 400]
        placeholders = ",".join("?" for _ in chunk)
        col.db.execute(f"delete from revlog where cid in ({placeholders})", *chunk)
    try:
        col.remove_config(DEMO_CONFIG_KEY)
    except Exception:
        pass
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--clear-demo",
        action="store_true",
        help="remove ALL synthetic demo data (notes, cards, and the synthetic "
        "reviews) from the collection and exit, instead of seeding — leaving a "
        "clean real-data-only profile",
    )
    ...
    print("=" * 78)
    if args.clear_demo:
        print("ReadyMCAT demo CLEANUP — removing all SYNTHETIC demo data.")
    else:
        print(
            "ReadyMCAT SYNTHETIC demo seeder — this writes FAKE data for a UI preview."
        )
    ...
    try:
        if args.clear_demo:
            removed = remove_demo_data(col)
            print(
                f"Removed {removed} synthetic demo notes (with their cards and "
                "reviews). The dashboard now reflects only real study data."
            )
            return 0
