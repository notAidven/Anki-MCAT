...
//
// Retrieve-before-reveal: on a FIRST miss the model answer / accepted answers /
// explanation are NOT shown — only the "start guiding questions" button (the
// typed answer is marked "Not quite" but the correct answer stays hidden). An
// item with no authored ladder (`subquestions` empty) falls back to the normal
// answer reveal so the student is never stranded.
    ...
    renderFrQuestion({
        ...
        afterSubmit: (_value, correct, wrap) => {
            ...
            if (correct) {
                ...
                // PRD: on a miss, DON'T hand over the answer (model answer /
                // accepted answers / explanation stay hidden). Show only the
                // button that rebuilds it with the item's own guiding
                // sub-questions (retrieval, not reading).
                addNote(
                    wrap,
                    "spaced",
                ...
                // No guiding ladder authored for this item: never strand the
