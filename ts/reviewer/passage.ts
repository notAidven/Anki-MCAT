...
async function typeset(el: HTMLElement): Promise<void> {
    try {
        ...
        // ignore math typesetting errors in the passage overlay
    }
}
...
function makeButton(
    ...
    button.className = `rmpsg-btn ${cls}`;
    button.type = "button";
    ...
    renderPassageQuestion({
        ...
        afterSubmit: (chosen, correct, wrap) => {
            ...
            if (correct) {
                ...
                // PRD: on a miss, DON'T hand over the explanation. Run the
                // question's own guiding sub-questions (retrieval, not reading).
                addNote(
                    wrap,
                    "spaced",
