...
import { firstUrl, hasLadder, makeButton, qaEl, typeset } from "./readymcat_dom";
...
describe("hasLadder", () => {
    test("is true only when the ladder has at least one rung", () => {
        expect(hasLadder([{ stem: "q" }])).toBe(true);
        expect(hasLadder([{ stem: "a" }, { stem: "b" }])).toBe(true);
    });

    test("is false for an empty, missing or non-array ladder", () => {
        expect(hasLadder([])).toBe(false);
        expect(hasLadder(undefined)).toBe(false);
        expect(hasLadder(null)).toBe(false);
