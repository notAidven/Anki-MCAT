...
<script lang="ts">
    ...
    // Per-topic first-attempt accuracy, keyed by category.
    $: accuracyByCat = new Map(perfTopics.map((t) => [t.category, t]));

    // Enriched topics for the interactive mastery ring grid (FSRS recall +
    // first-attempt accuracy merged per AAMC category) — the shared signature
    // visualization, also used by the home hub.
    $: masteryTopics = topics.map((t): MasteryTopic => {
        const p = accuracyByCat.get(t.category);
        return {
            category: t.category,
            name: t.name,
...
<div class="dashboard">
    ...
    {:else}
        ...
        <!-- Study next: the shared recommendation card, above the supporting
             detail so it reads as the primary "what to do now" on the dashboard
             too (same component + logic as the home hub). -->
        ...
        <section class="card" bind:this={masterySection} id="rmcat-dashboard-topics">
            <p class="section-note">
                Each ring fills to your recall for that AAMC topic. Sort by
                <strong>Focus</strong>
                to surface the highest
                <InfoTooltip entry={glossary.pointsAtStake}>
                    points at stake
                </InfoTooltip>
                (exam weight × your weakness) first, then click any ring for its detail. {rankedCount}
                due cards ranked.
            </p>
            <TopicMastery topics={masteryTopics} {focusRequest} />
        </section>
...
<style lang="scss">
    ...
    /* The shared "Study next" card carries its own surface styling; just space
       it like the other dashboard cards. */
    .studynext-slot {
        margin-bottom: 0.85rem;
    }
